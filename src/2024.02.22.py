...
def cha_cha_20_decrypt(p_key, p_nonce, p_ciphertext):
    cipher = ChaCha20.new(key=p_key, nonce=p_nonce)
    plaintext = cipher.decrypt(p_ciphertext)
    return plaintext


def main():
    print("This script encrypts and decrypts a String with the ChaCha20 Cipher.")
    ...
    decrypted_text = cha_cha_20_decrypt(key, nonce, ciphertext)
    print(f"Decrypted String: {decrypted_text.decode()}")
